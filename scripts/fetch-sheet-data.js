  ...
  try {
    ...
    if (credentialsPath) {
      try {
        ...
        console.error('Error loading credentials from file:', fileError.message);
        process.exit(1);
      }
    }
    // Option 2: Parse from JSON string (for CI/CD)
      ...
      try {
        ...
        console.error('Error parsing GOOGLE_SERVICE_ACCOUNT_KEY JSON:', parseError.message);
        console.error('Make sure the JSON is valid and properly formatted');
        process.exit(1);
      }
    }
    else {
      console.error('Error: Neither GOOGLE_SERVICE_ACCOUNT_KEY nor GOOGLE_SERVICE_ACCOUNT_PATH is set');
      console.error('Set one of these environment variables in your .env.local file');
      process.exit(1);
    }
    ...
    const fetchPromises = sheetNames.map(sheetName => 
      sheets.spreadsheets.values.get({
        ...
        .catch(error => {
          console.error(`  ⚠ Warning: Could not fetch sheet "${sheetName}": ${error.message}`);
...
function transformToGuidebookFormat(sheetsData) {
  ...
  guidebook.checkInOut = {
    checkIn: {
      ...
      steps: sheetsData['check-in-steps']?.map(row => row.stepDescription) || []
    },
    checkOut: {
      title: "Check-out Process",
      steps: sheetsData['check-out-steps']?.map(row => row.stepDescription) || []
    },
      ...
      if (key === 'parking') {
        ...
        if (row.info && row.info.trim()) {
          try {
            ...
            console.warn('Failed to parse airport transfer options JSON:', e.message);
          }
        }
  ...
  if (sheetsData['house-rules']) {
    guidebook.houseRules = {
      ...
      importantNote: {
        ...
        message: "This apartment is subject to body corporate by-laws. Violations may result in fines, immediate eviction without refund, and loss of security bond. Please respect all residents and building rules."
      }
    };
  }
  ...
  if (sheetsData['emergency-contacts']) {
    guidebook.emergency = {
      alert: {
        ...
        message: "For life-threatening emergencies, always call <strong>000</strong> (Triple Zero) for Police, Fire or Ambulance."
      },
      ...
      addressNote: "Address for Emergency Services: Broadbeach, Gold Coast, Queensland 4218 (provide specific unit number when calling)"
    };
  }
